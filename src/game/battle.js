...
const HERO_SELECT = 'id, level, xp, current_health, troops_data, troop_level, is_defender, skin, weapon, template:character_templates (id, name, base_attack, base_defense, base_health, rarity, troop_type, troop_power, troops_per_level)';
...
async function getAttackHeroes(t) {
  const db = getSupabase(); const listed = await getListedHeroIds(t);
  const { data } = await db.from('player_characters').select(HERO_SELECT).eq('telegram_id', t).gt('current_health', 0).eq('is_defender', false);
...
function calcTeamPower(heroes) {
  return heroes.reduce((sum, h) => {
    ...
    return sum + t.base_attack + (sb.attack || 0) + t.base_defense + (sb.defense || 0) + h.level * 5 + heroTroopsPower(h) + weaponPower(h.weapon);
  }, 0);
}
...
async function fightNPC(telegramId, botRealm, selectedHeroIds) {
  ...
  if (playerWins) {
    ...
    for (const hero of selected) {
      ...
      const newXp = (hero.xp || 0) + xpGained; const xpNeeded = hero.level * 100;
      const leveledUp = newXp >= xpNeeded; const newLevel = leveledUp ? hero.level + 1 : hero.level;
      const finalXp = leveledUp ? newXp - xpNeeded : newXp;
      const damage = Math.floor(Math.random() * 10) + 5; const newHp = Math.max(1, hero.current_health - damage);
    ...
    for (const hero of selected) {
      ...
      if (newHp <= 0) { await db.from('player_characters').delete().eq('id', hero.id); deadHeroes.push(hero.template.name); }
      else await db.from('player_characters').update({ current_health: newHp, troops_data: {} }).eq('id', hero.id);
    }
