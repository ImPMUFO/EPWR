...
async function findPvPTargets(attackerId) {
  ...
  const { data } = await db.from('players').select('telegram_id, commander_name, level, gold').neq('telegram_id', attackerId).gt('level', 0).limit(5);
  return data || [];
}

function calcPower(heroes) {
  return heroes.reduce((s, h) => s + h.template.base_attack + h.template.base_defense + h.level * 5 + heroTroopsPower(h), 0);
}
...
async function executePvP(attackerId, defenderId, selectedHeroIds) {
  ...
  for (const hero of selected) {
    const damage = Math.floor(Math.random() * 15) + 5; const newHp = hero.current_health - damage;
  ...
  if (attackerWins) {
    for (const dh of defenderHeroes.slice(0, 3)) {
      const damage = Math.floor(Math.random() * 20) + 10; const newHp = dh.current_health - damage;
  ...
  await db.from('pvp_battles').insert({ attacker_id: attackerId, defender_id: defenderId, winner_id: winnerId, gold_stolen: goldStolen, attacker_power: attackerPower, defender_power: defenderPower });
  ...
  if (attackerWins) await addNotification(defenderId, 'attack', `${attacker.commander_name} به شما حمله کرد و پیروز شد!`, attacker.commander_name, goldStolen);
