const { getAdminId, claimAdmin, isAdmin, createGiftCode, getAllGiftCodes, toggleGiftCode, deleteGiftCode, addResources } = require('../../game/gift');
...
module.exports = function registerAdmin(bot) {
  ...
  // ═══ ورود به پنل (با ثبت خودکار مدیر) ═══
  bot.command('admin', async (ctx) => {
    let adminId = await getAdminId();
    if (!adminId) { await claimAdmin(ctx.from.id); adminId = ctx.from.id; await ctx.reply('👑 شما به عنوان مدیر ثبت شدید!'); }
    if (ctx.from.id !== adminId) return ctx.reply('⛔ شما دسترسی ندارید!');
  ...
  bot.action(/^ah_create\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    getState(ctx.from.id).step = 'create_name'; getState(ctx.from.id).data = {};
    await ctx.reply('⚜️ *ساخت قهرمان*\n\nنام قهرمان رو تایپ کن:', { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: 'لغو', callback_data: cb('admin_heroes', ctx.from.id) }]] } });
  });

  // ویرایش متنی
  bot.action(/^ah_f_(\w+)_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    const field = ctx.match[1], idx = parseInt(ctx.match[2]);
    const names = { name:'نام', base_health:'سلامتی', base_attack:'حمله', base_defense:'دفاع', price_gold:'قیمت سکه', price_gems:'قیمت الماس', troop_power:'قدرت سرباز', troops_per_level:'ظرفیت سرباز در هر لول' };
    getState(ctx.from.id).step = 'edit_field'; getState(ctx.from.id).data = { field, idx };
    await ctx.reply(`✏️ *${names[field] || field}* جدید رو تایپ کن:`, { parse_mode: 'Markdown' });
  });

  // انتخاب‌های enum
  bot.action(/^ah_rarity_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    const i = parseInt(ctx.match[1]), uid = ctx.from.id;
    await ctx.reply('🎭 کمیابی رو انتخاب کن:', { reply_markup: { inline_keyboard: [
      [{ text: 'معمولی', callback_data: `ah_setrarity_common_${i}|${uid}` }, { text: 'کمیاب', callback_data: `ah_setrarity_rare_${i}|${uid}` }],
      [{ text: 'حماسی', callback_data: `ah_setrarity_epic_${i}|${uid}` }, { text: 'افسانه‌ای', callback_data: `ah_setrarity_legendary_${i}|${uid}` }]
    ] } });
  });
  bot.action(/^ah_setrarity_(\w+)_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    await setHeroField(ctx, parseInt(ctx.match[2]), { rarity: ctx.match[1] });
  });

  bot.action(/^ah_type_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    const i = parseInt(ctx.match[1]), uid = ctx.from.id;
    await ctx.reply('🗡 نوع قهرمان رو انتخاب کن:', { reply_markup: { inline_keyboard: [
      [{ text: 'نزدیک‌زن', callback_data: `ah_settype_melee_${i}|${uid}` }, { text: 'دورزن', callback_data: `ah_settype_ranged_${i}|${uid}` }],
      [{ text: 'جادویی', callback_data: `ah_settype_magic_${i}|${uid}` }, { text: 'سواره', callback_data: `ah_settype_cavalry_${i}|${uid}` }]
    ] } });
  });
  bot.action(/^ah_settype_(\w+)_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    await setHeroField(ctx, parseInt(ctx.match[2]), { type: ctx.match[1] });
  });

  bot.action(/^ah_troop_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    const i = parseInt(ctx.match[1]), uid = ctx.from.id;
    await ctx.reply('🪖 نوع سرباز اختصاصی رو انتخاب کن:', { reply_markup: { inline_keyboard: [
      [{ text: 'نیزه‌دار', callback_data: `ah_settroop_spear_${i}|${uid}` }, { text: 'کماندار', callback_data: `ah_settroop_archer_${i}|${uid}` }],
      [{ text: 'شوالیه', callback_data: `ah_settroop_knight_${i}|${uid}` }, { text: 'منجنیق', callback_data: `ah_settroop_catapult_${i}|${uid}` }]
    ] } });
  });
  bot.action(/^ah_settroop_(\w+)_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    await setHeroField(ctx, parseInt(ctx.match[2]), { troop_type: ctx.match[1] });
  });

  bot.action(/^ah_hide_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    const i = parseInt(ctx.match[1]);
    const chars = await getChars(); const h = chars[i];
    await setHeroFieldRaw(ctx, h.id, { hidden: !h.hidden });
  });

  bot.action(/^ah_img_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    const chars = await getChars(); const h = chars[parseInt(ctx.match[1])];
    getState(ctx.from.id).step = 'img_hero'; getState(ctx.from.id).data = { heroId: h.id };
    await ctx.reply(`📸 تصویر *${h.name}* رو بفرست:`, { parse_mode: 'Markdown' });
  });

  bot.action(/^ah_del_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    const i = parseInt(ctx.match[1]), uid = ctx.from.id;
    await ctx.reply('⚠️ این قهرمان برای همیشه حذف میشه. مطمئنی؟', { reply_markup: { inline_keyboard: [
      [{ text: 'بله، حذف کن', callback_data: `ah_delconfirm_${i}|${uid}` }],
      [{ text: 'خیر', callback_data: `ah_edit_${i}|${uid}` }]
    ] } });
  });
  bot.action(/^ah_delconfirm_(\d+)\|(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return;
    const chars = await getChars(); const h = chars[parseInt(ctx.match[1])];
    const db = getSupabase();
    await db.from('player_characters').delete().eq('template_id', h.id);
    await db.from('character_templates').delete().eq('id', h.id);
    await ctx.answerCbQuery('🗑 حذف شد', { show_alert: true });
    await showHeroesList(ctx);
  });
  ...
  bot.action(/^admin_add_resources\|(\d+)$/, async (ctx) => { await ctx.answerCbQuery(); if (!(await isAdmin(ctx.from.id))) return; getState(ctx.from.id).step='resource_user_id'; getState(ctx.from.id).data={}; await ctx.reply('💰 آیدی عددی کاربر رو بفرست:', { reply_markup: { inline_keyboard: [[{ text: 'لغو', callback_data: cb('admin', ctx.from.id) }]] } }); });
  ...
  bot.on('text', async (ctx, next) => {
    ...
    if (s.step==='create_name') {
      const db = getSupabase();
      const { data } = await db.from('character_templates').insert({
        name: text, type:'melee', rarity:'common', base_health:100, base_attack:20, base_defense:10,
        price_gold:500, price_gems:0, troop_type:'spear', troop_power:2, troops_per_level:2, hidden:false
      }).select().single();
      clearState(ctx.from.id);
      await ctx.reply(`✅ *${text}* ساخته شد! حالا مشخصاتش رو تنظیم کن.`, { parse_mode:'Markdown' });
      const chars = await getChars();
      const idx = chars.findIndex(c=>c.id===data.id);
      await showHeroEdit(ctx, idx);
      return;
    }

    if (s.step==='edit_field') {
      const { field, idx } = s.data;
      const isNum = field!=='name';
      const value = isNum ? (parseInt(text)||0) : text;
  ...
  async function setHeroFieldRaw(ctx, heroId, updates) {
    ...
    await ctx.answerCbQuery('✅ ذخیره شد', { show_alert: false });
    const chars = await getChars();
  ...
  async function showHeroesList(ctx) {
    ...
    for (let i=0;i<chars.length;i+=2){
      const row=[];
      row.push({ text: chars[i].name, callback_data: `ah_edit_${i}|${uid}` });
      if(i+1<chars.length) row.push({ text: chars[i+1].name, callback_data: `ah_edit_${i+1}|${uid}` });
      buttons.push(row);
    }
  ...
  async function showHeroEdit(ctx, idx) {
    const chars = await getChars(); const h = chars[idx]; if(!h) return;
    ...
    await ctx.reply(msg, { parse_mode:'Markdown', reply_markup:{ inline_keyboard: [
      ...
      [{ text:'ظرفیت سرباز', callback_data:`ah_f_troops_per_level_${idx}|${uid}` }, { text:'تصویر', callback_data:`ah_img_${idx}|${uid}` }],
  ...
  async function showGroups(ctx) {
    const db=getSupabase(); const {data:g}=await db.from('bot_groups').select('*');
  ...
  async function showStats(ctx) {
    ...
    const {count:bc}=await db.from('battles').select('*',{count:'exact',head:true});
  ...
  async function showGiftList(ctx) {
    const codes=await getAllGiftCodes(); const uid=ctx.from.id;
    if(codes.length===0)return ctx.reply('📭 خالی',{reply_markup:{inline_keyboard:[[{text:'ساخت',callback_data:cb('admin_create_gift',uid)}],[{text:'بازگشت',callback_data:cb('admin',uid)}]]}});
    let msg='🎁 *کدها*\n\n'; const buttons=[];
    codes.forEach(c=>{ msg+=`${c.is_active?'🟢':'🔴'} \`${c.code}\` 💰${c.gold_reward} 💎${c.gems_reward}\n`; buttons.push([{text:c.is_active?'غیرفعال':'فعال',callback_data:`admin_toggle|${c.id}|${uid}`},{text:'حذف',callback_data:`admin_delete|${c.id}|${uid}`}]); });
    buttons.push([{text:'بازگشت',callback_data:cb('admin',uid)}]);
    await ctx.reply(msg,{parse_mode:'Markdown',reply_markup:{inline_keyboard:buttons}});
  }
