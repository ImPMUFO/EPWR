...
module.exports = function registerWorld(bot) {
  ...
  bot.action(/^world\|(\d+)$/, async (ctx) => { await ctx.answerCbQuery(); await showWorld(ctx); });
  ...
  async function showWorld(ctx) {
    ...
    let msg = `🗺️ *نقشه جهان*\n👥 ${count || 0} | 🏆 ${defeated.length}/${bots.length}\n\n`;
    const buttons = [];
    bots.forEach(b => {
      if (defeated.includes(b.id)) msg += `✅ ~~${b.emoji} ${b.name}~~\n`;
      else {
