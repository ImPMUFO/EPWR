...
module.exports = function registerShop(bot) {
  ...
  bot.action(/^hero:(.+):(\d+)$/, async (ctx) => {
    ...
    if (hero.current_health < maxHp) {
      buttons.push([{ text: '🧪 معجون', callback_data: `use_potion:${hero.id}:${uid}` }]);
    }
    buttons.push([
  ...
  async function showShop(ctx) {
    ...
    if (characters.length > 0) {
      ...
      for (let i = 0; i < characters.length; i += 2) {
        ...
        if (i + 1 < characters.length) {
          ...
          row.push({ text: `${rarityEmoji(c2.rarity)} ${c2.name} ${p2}`, callback_data: `buy_char:${c2.id}:${uid}` });
        }
        buttons.push(row);
      }
      msg += `\n`;
    ...
    if (items.length > 0) {
      ...
      for (let i = 0; i < items.length; i += 2) {
        ...
        if (i + 1 < items.length) {
          row.push({ text: `📦 ${items[i+1].name} 💰${items[i+1].price_gold}`, callback_data: `buy_item:${items[i+1].id}:${uid}` });
        }
        buttons.push(row);
      }
    }
