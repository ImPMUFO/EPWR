...
module.exports = function registerQuest(bot) {
  ...
  bot.action(/quest:uid:(\d+)/, async (ctx) => {
    await ctx.answerCbQuery();
    await assignDailyQuests(ctx.from.id);
    await showQuests(ctx);
  });

  bot.action(/^claim_quest:(.+):uid:(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery();
    const result = await claimQuestReward(ctx.from.id, ctx.match[1]);
    if (result.success) {
      await ctx.answerCbQuery(`🎁 +${result.gold}💰 +${result.gems}💎`, { show_alert: true });
    } else {
  ...
  async function showQuests(ctx) {
    ...
    quests.forEach(q => {
      ...
      if (q.is_completed && !q.is_claimed) {
        buttons.push([{ text: `🎁 دریافت`, callback_data: `claim_quest:${q.id}:uid:${uid}` }]);
      }
    });
